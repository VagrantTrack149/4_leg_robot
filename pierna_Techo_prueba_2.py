...
estado= {
    ...
    # Puntos de control del Bézier cúbico vigente: P0 (origen), P1, P2
    ...
    'segmento_bloqueado': False,   # True si el segmento actual no es viable (colisión o fuera de rango)
...
def preparar_segmento(p0, p3):
    ...
    while bezier_colisiona(p0, p1, p2, p3):
        ...
        if elevacion > elevacion_max:
            # No fue posible esquivar el escalón solo con Z
            return p1, p2, True

        p1, p2= puntos_control_bezier(p0, p3, elevacion)

    # Curva libre de colision verificar ángulos articulares en todo
...
def iniciar_trayectoria():
    ...
    if bloqueado:
        ...
        print(f"Ruta no iniciada: el primer segmento (punto 0) no es alcanzable "
              f"(colisión sin solucion o angulos fuera de rango).")
    else:
...
def actualizar(frame):
            ...
            if estado['indice_actual'] >= len(estado['trayectoria']):
                ...
                if bloqueado:
                    ...
                    print(f"Ruta detenida en el punto {estado['indice_actual']}: "
                          ...
                          f"(colisión sin solución o ángulos fuera de rango).")
                    if not estado['ruta_terminada']:
                        estado['ruta_terminada']= True
                        mostrar_reporte_error()
    ...
    # Actualizar la curva Bézier vigente (4 puntos de control)
