...
funcion_control = ax.plot(
    [], [], [],
    '-', color='orange', linewidth=1.5, label='CONTROL PD'
)
print("error = estado['objetivo'] - estado['actual'] aceleracion = KP*error - KD*estado['velocidad']  estado['velocidad'] += aceleracion * dt  estado['actual'] += estado['velocidad'] * dt")
...
def mostrar_reporte_error():
    ...
    ax_comp.plot(registro['indices_alcanzados'], planeados[:, 1], 'o--', color='mediumblue', label='y planeado')
    ax_comp.plot(registro['indices_alcanzados'], alcanzados[:, 1], 'x-', color='darkorange', label='y real')
...
def actualizar(frame):
    ...
    aceleracion = KP*error - KD*estado['velocidad']

    estado['velocidad'] += aceleracion * dt

    estado['actual'] += estado['velocidad'] * dt
